...
1;;){var k=b.indexOf(d,f),f=b.indexOf(a,f);if(0>f)break;if(0>k||f<k){if(0===c){j.end=f;break}else c--;f+=1}else c++,f=k+1}return 0<=j.start&&0<=j.end?j:null},K1K:function(b,c){for(var d=0,a=0,j=[],l=[],k=c.length;d<b.length;){if(g[b[d]])l.push(b[d]);else if(e[b[d]]){var m=l.pop();f[m]!==b[d]&&(h.Log.warning("Unclosed block is detected.\r"+b),l.push(m))}else 0===l.length&&(b[d]===c[0]&&b.substr(d,k)===c)&&(j.push(b.substr(a,d-a)),a=d+k);d++}0<l.length&&h.Log.warning("Unclosed block is detected.\r"+
...
for(a=0;a<d.length;a++)try{d[a].callback.apply(g,[e,f,b])}catch(l){h.Log.warning("Call changed callback failed.",l)}}},getPropertiesChangeRecords:function(g){g=h.KJ.KK(g);return!g?null:g.changedProperties?g.changedProperties:[]},clearPropertiesChangeRecords:function(g){g=h.KJ.KK(g);if(!g)return null;g.changedProperties=null},KP:function(g,e){if("*"!==e){var f=h.KJ.KK(g);f.dataHookInfo||(f.dataHookInfo={hookedProperties:[],
...
d.splice(d.indexOf(a),1))},K8:{},K9:function(a,b){this.KD(a,b);var d=b.provider.getValue(a,b.description,b.options);try{if(b.pipes){for(var c=0;c<b.pipes.length;c++){var e=f.K1F.getValueOnPath(a,b.pipes[c]);"function"!==typeof e&&f.Log.error("Pipe must be a function. pipe name:"+b.pipes[c]);d=e.apply(a,[d])}b.errorAPProvider&&b.errorAPProvider.setValue(a,"!"+b.description,void 0,b.options);f.Debugger.errorStatusChanged(a,b,void 0)}}catch(g){return b.errorAPProvider&&
...
c,g)}finally{delete b.KB}h(a,b.options,"@change",arguments)}}},b.provider.monitor(a,b.description,b.changedCallback,b.options))},stopMonitoring:function(a,b){this.KD(a,b);b.changedCallback&&b.provider.doesSupportMonitoring(a,b.description)&&(b.provider.stopMonitoring(a,b.description,b.changedCallback,b.options),delete b.changedCallback)},KE:function(a,b,d,c,e){var g,i,o,p;a.isComposite?(g=a,i=b,o=d,p=c):(g=d,i=c,o=a,p=b);for(a=0;a<g.childrenAPs.length;a++)g.childrenAPs[a].provider=
f.K3.K4(i,g.childrenAPs[a].description);p.provider=f.K3.K4(p,o.description);g.changedCallback=function(){for(var a=[],d=0;d<g.childrenAPs.length;d++){var l=f.K3.K9(i,g.childrenAPs[d]);if(l===f.K3.K8)return;a.push(l)}d=f.K1F.getValueOnPath(i,g.nToOnePipe);typeof d!=="function"&&f.Log.error("Pipe must be a function. pipe name:"+g.nToOnePipe);a=d.apply(i,[a]);f.K3.KC(b,c,e,a,p===c);f.K3.KA(p,
o,a);h(p,o,"@change",[b,c,e])};for(a=0;a<g.childrenAPs.length;a++)g.childrenAPs[a].provider.doesSupportMonitoring(i,g.childrenAPs[a].description)&&g.childrenAPs[a].provider.monitor(i,g.childrenAPs[a].description,g.changedCallback,g.childrenAPs[a].options);g.changedCallback()},KF:function(a,b,d){var c=g(a,b,d.leftAP,!0),h=g(a,b,d.rightAP,!1),a=c.target,b=h.target,c=c.ap,h=h.ap;if(c.isComposite||h.isComposite)this.KE(c,a,h,b,d);else{this.KD(a,c);this.KD(b,h);var i;
...
0;c<d.length;c++){a.__knot||(a.__knot={options:[]});var e=k.K18.parse(d[c]);0!==e.length&&(a.__knot.options.push(e[0]),b(e[0].leftAP)&&0>m.indexOf(a)&&m.push(a))}if(a.children)for(d=0;d<a.children.length;d++)g(a.children[d])}function e(a){for(var b=0;b<a.length;b++){var d=a[b].scope,c=a[b].CBS,e=void 0,f=void 0;for(f in c)if(k.K1F.K1G(f,"/")){for(var e=f,j=c,h=[],l=0;l<j[e].length;l++)h=h.concat(k.K18.parse(j[e][l]));k.Kh.addGlobalObjectOption(e,h)}else{var i;
try{i=d.querySelectorAll(f)}catch(m){k.Log.warning("Invalid selector:"+f,m);continue}0===i.length&&k.Log.warning("There is no element selected with selector:"+f);for(e=0;e<i.length;e++)j=c[f].slice(0),i[e].__knot||(i[e].__knot={options:[]}),i[e].__knot.cbsOptions&&(j=i[e].__knot.cbsOptions.concat(j)),i[e].__knot.cbsOptions=j}a[b].scope===document?g(document.body):g(a[b].scope)}}function f(a,b){var d=[],c;b||(c={scope:document,CBS:{},isGlobalScope:!0},d.push(c));for(var e=0;e<a.length;e++){var f;var j=
a[e];if(!j||""===j)f=null;else{for(var g=void 0;0<=(g=j.indexOf("/*"));){f=j.indexOf("*/",g);if(0>f)throw Error("Can't find close mark for comment.");j=j.substr(0,g)+j.substr(f+2)}j=j.split("\n");g="";for(f=0;f<j.length;f++)for(var l=j[f].split("\r"),i=0;i<l.length;i++)"//"!==k.K1F.trim(l[i]).substr(0,2)&&(g+=l[i]);f=g}f=k.K1F.trim(f);if(k.K1F.K1G(f,"$private")){j={CBS:{}};f=f.substr(8);g=j;g.HTML||(g.HTML="");for(var l=0,i="",m=k.K1F.K1J(f,l,"<<{{","}}>>");null!==
m;)i+=f.substr(l,m.start-l),g.HTML+=f.substr(m.start+4,m.end-m.start-4),l=m.end+4,m=k.K1F.K1J(f,l,"<<{{","}}>>");l<f.length&&(i+=f.substr(l));f=i;try{j.scope=document.createElement("DIV"),j.scope.innerHTML=j.HTML,delete j.HTML}catch(n){k.Log.error("Load private scope HTML error.",n);continue}h(j.CBS,f);d.push(j)}else b?k.Log.error("Can't load CBS. It must be a private CBS!"):h(c.CBS,f)}return d}function b(a){return"foreach"===a.description||"content"===a.description}function c(a){for(var a=
...
a.length;e++)f=f.concat(c(a[e].scope));for(e=0;e<m.length;e++)for(var j=0;j<m[e].__knot.options.length;j++)b(m[e].__knot.options[j].leftAP)&&d(m[e],m[e].__knot.options[j].leftAP),b(m[e].__knot.options[j].rightAP)&&d(m[e],m[e].__knot.options[j].rightAP);for(e=m.length=0;e<f.length;e++)delete f[e].__knot_template_id,f[e].removeAttribute("knot-template"),f[e].removeAttribute("knot-template-id"),f[e].parentNode.removeChild(f[e]);for(e=0;e<a.length;e++)a[e].isGlobalScope||0<a[e].scope.children.length&&
k.Log.warning("Useless HTML is detected in private scope.")}function j(a){n.push(a);var b=new k.Deferred,d=k.K1F.K1I();d.onreadystatechange=function(){4===d.readyState&&(200===d.status?b.resolve(d.responseText):b.reject(Error("Load CBS script error. url:"+a+" message:"+d.statusText)))};d.open("GET",a,!0);d.send();return b}function l(){for(var a=new k.Deferred,b=i.document.querySelectorAll('script[type="text/cbs"]'),d=[],c=0,e=function(){if(0===c)try{a.resolve(d)}catch(b){k.Log.error("Initialize failed.  message:"+
b.message,b)}},f=0;f<b.length;f++)b[f].src?0<=n.indexOf(b[f].src)||(c++,function(){var a=b[f].src;j(a).done(function(a){d.push(a);c--;e()},function(b){c--;k.Log.error("Load CBS script error. url:"+a,b);e(b)})}()):d.push(b[f].textContent);0===c&&a.resolve(d);return a}var k=i.Knot.K1M(),m=[],q=0,n=[];k.K1Q={loadGlobalScope:function(){var b=new k.Deferred;l().done(function(d){try{var c=f(d,!1);e(c);a(c);b.resolve()}catch(j){k.Log.error("Initialize failed.  message:"+j.message,j),b.reject(j)}},
function(a){b.reject(a)});return b},loadCBSPackage:function(b){var d=new k.Deferred;0<=n.indexOf(b)?d.resolve():j(b).done(function(c){try{var j=f([c],!0);e(j);a(j)}catch(g){k.Log.error("Parse CBS package failed. package file:"+b,g),d.reject(g)}d.resolve()},function(a){d.reject(a)});return d},test:{parseCBSInDocument:l,getScopes:f,Ks:e,Kv:a}}})(window);(function(i){function h(d){b.Kh.K15(d);d.parentNode.removeChild(d);b.Debugger.nodeRemoved(d)}function g(d,a,c){if(!c||!c.template)b.Log.error("No valid template is specified for 'content' access point. current node:"+b.KX.KY(d));else{var e=function(f,e){if(c&&c[e]){var g=b.K1F.getValueOnPath(a,c[e]);try{g.apply(d,[f,a])}catch(h){b.Log.warning("Raise "+e+" event failed.",h)}}},f=d.childNodes[0];if(f){if(!(f.__knot&&f.__knot.dataContext===a))if(null===
...
b.Kh.hasDataContext(f)||b.Kh.setDataContext(f,a),e(f,"@added"),b.Debugger.nodeAdded(f)}}function e(d,a,c,f,e){a||(a=[]);for(var g=0;g<a.length;g++){var i;a:{for(var n=g;n<d.children.length;n++)if(d.children[n].__knot&&d.children[n].__knot.dataContext===a[g]){i=d.children[n];break a}i=null}if(i){if(Array.prototype.indexOf.call(d.children,i)!==g){d.removeChild(i);var n=d,o=g;n.children.length===o?n.appendChild(i):n.insertBefore(i,n.children[o])}}else if(n=b.Kh.createFromTemplate(c,
a[g],d)){i=d;var o=n,p=g;i.children.length===p?i.appendChild(o):i.insertBefore(o,i.children[p]);b.Kh.hasDataContext(n)||b.Kh.setDataContext(n,a[g]);b.Debugger.nodeAdded(n);f&&f.apply(d,[n,a[g]])}}for(g=d.children.length-1;g>=a.length;g--)h(d.children[g]),e&&e.apply(d,[d.children[g]])}function f(d,a,c){!c||!c.template?b.Log.error("No valid template is specified for 'foreach' access point. current node:"+b.KX.KY(d)):e(d,a,c.template,b.K1F.getValueOnPath(d,
...
c.__knot_errorStatusInfo&&(c.__knot_errorStatusInfo[a]&&c.__knot_errorStatusInfo[a].K1O)&&(e=c.__knot_errorStatusInfo[a].K1O.indexOf(e),0<=e&&c.__knot_errorStatusInfo[a].K1O.splice(e,1))},getErrorStatusInformation:function(b,a){if(b.__knot_errorStatusInfo)for(var c in b.__knot_errorStatusInfo)b.__knot_errorStatusInfo[c]&&b.__knot_errorStatusInfo[c].currentStatus&&a.push({node:b,accessPointName:c,error:b.__knot_errorStatusInfo[c].currentStatus});for(c=0;c<b.children.length;c++)this.getErrorStatusInformation(b.children[c],
...
c){if(d){for(var a=[],c=b.className.split(" "),d=d.split(" "),i=0;i<d.length;i++){var l=d[i];"-"===l[0]?(l=l.substr(1).trim(),0<=c.indexOf(l)&&c.splice(c.indexOf(l),1)):"+"===l[0]?(l=l.substr(1).trim(),0>c.indexOf(l)&&c.push(l)):a.push(l)}b.className=0<a.length?a.join(" "):c.join(" ")}}else if("select"===b.tagName.toLowerCase()&&"value"===c){for(a=0;a<b.options.length;a++)if(b.options[a].value==d){b.selectedIndex=a;return}b.selectedIndex=-1}else if("select"===b.tagName.toLowerCase()&&"selectedData"===
c)a:{for(a=0;a<b.options.length;a++)if(b.options[a].__knot&&b.options[a].__knot.dataContext===d){b.selectedIndex=a;break a}b.selectedIndex=-1}else if("select"===b.tagName.toLowerCase()&&e.K1F.K1G(c,"options")){(c=a)||(c={});d||(d=[]);for(a=0;a<d.length;a++){a:{for(var i=b.options,l=d[a],k=a;k<i.length;k++)if(e.Kh.K1L(i[k])===l){i=i[k];break a}i=null}i?b.options[a].indexOf(i)!==a&&(b.options.remove(b.options[a].indexOf(i)),b.options.add(i,a)):(i=document.createElement("option"),
e.Kh.K16(i,d[a]),d[a]&&(c.displayMember?h(i,"text",d[a],c.displayMember):i.text=d[a],c.valueMember?h(i,"value",d[a],c.valueMember):i.value=d[a]),b.options.add(i,a))}for(a=b.options.length-1;a>=d.length;a--){c=b.options[a];if(c.__knot&&c.__knot.options){for(i=0;i<c.__knot.options.length;i++)e.K3.KG(c,c.__knot.dataContext,c.__knot.options[i]);delete c.__knot.dataContext}b.options.remove(a)}}else f[c]&&(a=c,"undefined"===typeof d&&(d=""),c=g(b),f[a][c]?b[f[a][c]]=
...
"$hash.originalHash",a)}finally{c=!1}}}var e=i.Knot.K1M(),f,b,c=!1;e.K1F.setValueOnPath(null,"$hash",{});var d=e.K1F.getValueOnPath(null,"$hash");g();e.K1P={setHashFormat:function(a,c){if(!(a instanceof Array))throw Error("Hash fields must be an array.");if(0<=a.indexOf("hash"))throw Error("Hash field can't be 'hash'.");if(!c)throw Error("Please specify the splitter.");var i;if(f)for(i=0;i<f.length;i++)e.KM.stopMonitoring(d,f[i],h);f=a;b=c;if(f)for(i=0;i<
...
i.Knot.clear=function(b){g.Kh.clear(b)};i.Knot.getDataContext=function(b){return g.Kh.K14(b)};i.Knot.findElementBindToData=function(b,c){for(var d=0;d<b.length;d++)if(i.Knot.getDataContext(b[d])===c)return b};i.Knot.setHashFormat=function(b,c){g.K1P.setHashFormat(b,c)};i.Knot.getKnotVariant=function(b){return g.K1F.getValueOnPath(null,b)};i.Knot.setKnotVariant=function(b,c){return g.K1F.setValueOnPath(null,b,c)};var e=[],f;i.Knot.ready=
function(b){e.push(b);(i.Knot.isReady||f)&&h()};i.Knot.isReady=!1;i.addEventListener("load",function(){g.K1Q.loadGlobalScope().done(function(){g.Kh.bind();i.Knot.isReady=!0;f=null;h()},function(b){i.Knot.isReady=!1;f=b;h()})})})(window);
